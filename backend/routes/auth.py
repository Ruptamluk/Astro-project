...
async def register(request_data: RegisterRequest, db=Depends(get_db)):
    ...
    return {
        "success": True,
        "message": f"OTP sent to {request_data.email} and {request_data.phone}. Please verify to complete registration.",
...
async def verify_otp(request_data: VerifyOTPRequest, db=Depends(get_db)):
    ...
    if not user:
        stored_phone = request_data.phone or otp_record.get("phone")
        is_registration = otp_record.get("type") == "registration"
        user_data = {
            "name": otp_record.get("name"),
            "email": request_data.email,
            "phone": stored_phone,
            # For registration OTPs, both email and phone are verified together
            "email_verified": bool(request_data.email),
            "phone_verified": is_registration and bool(stored_phone),
            "dob": None,
            "zodiac_sign": None,
            "tokens": 0,
            "created_at": datetime.utcnow()
